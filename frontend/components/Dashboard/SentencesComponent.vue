...
import type authFormComponentVue from '../authFormComponent.vue';
<script lang="ts" setup>
const { data: result, pending, error } = useFetch(() => `http://localhost:5555/api/sentences`, {
    method: 'get',
    headers: {
...
function formatTimeString(timeString) {
    ...
    const formattedDateString = `${year}-${month < 10 ? '0' : ''}${month}-${day < 10 ? '0' : ''}${day} ${hours < 10 ? '0' : ''}${hours}:${minutes < 10 ? '0' : ''}${minutes}:${seconds < 10 ? '0' : ''}${seconds}`;
            ...
            <table class="table table-borderless" v-if="result">
                ...
                <tbody>
                    <tr v-for="(sentence, index) in result.sentences" :key="index">
                        ...
                        <td>{{ sentence.text }}</td>
                        <td><NuxtLink :to="'users/' + sentence.author">{{ sentence.author }}</NuxtLink></td>
                        <td>
                            <ul v-if="sentence.translations.length">
                                <li v-for="(translation, indexTranslations) in sentence.translations" :key="indexTranslations">
                        ...
                        <td>{{ sentence.language === 'ru' ? 'Русский' : sentence.language }}</td>
                        <td>{{ formatTimeString(sentence.createdAt) }}</td>
                    </tr>
                </tbody>
            </table>
            <p>{{ result.message }}: {{ result.count }}</p>
        </div>
    </div>
</template>
