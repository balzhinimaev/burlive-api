...
export default async function greeting(ctx: rlhubContext) {
  try {
    const extra: ExtraEditMessageText = {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [
            {
              text: "Указать электронную почту",
              callback_data: "date_birth",
            },
          ],
          [
    ...
    if (!response.ok) {
      console.error("Failed to save user fetch personal data:", response.statusText);
    }
    ...
    message += `\n\nДата регистрации: <b>${await get_date(
      user?.user.createdAt
    )}</b>`;
    message += `\n\nE-mail: <b>${
      user.email ? user.email : `<a href="/set">указать e-mail</a>`
    }</b>`;
