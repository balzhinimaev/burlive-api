...
dashboard.start(async (ctx: rlhubContext, next) => {
  try {
    await ctx.reply("притвет");
    return await saveSceneMiddleware(ctx, next);
  } catch (error) {
    console.log(error);
  }
});
...
dashboard.enter(async (ctx: rlhubContext, next) => {
  try {
    ...
    console.log(error);
    return ctx.reply("Возникла ошибка, свяжитесь пожалуйста с @frntdev");
  }
});

dashboard.action("referral", async (ctx: rlhubContext) => {
  try {
    let message = `<b>🎉 Приглашайте друзей и получайте бонусы!</b> \n`;
    message += `\nПоделитесь своей реферальной ссылкой и получайте награды за каждого нового пользователя. Поддержите проект и станьте частью нашего сообщества!`;
    message += `\n\n<a href="https://telegra.ph/Priglashajte-druzej-i-poluchajte-bonusy-06-13">Подробная информация про партнерскую программу</a>`;
    message += `\n\nВаша реферальная ссылка: <code>https://t.me/burlive_bot?start=ref_${ctx.from?.id}</code>`;
    const extra: ExtraEditMessageText = {
      parse_mode: "HTML",
      reply_markup: {
  ...
  if (!isNaN(pageNumber)) {
    try {
      ...
      ctx.replyWithPhoto(
        ...
        {
          reply_markup: {
            inline_keyboard: [[{ callback_data: "back", text: "Назад" }]],
          },
        }
      );
    } catch (error) {
...
dashboard.action(
  "about",
  async (ctx) => await about_project_section_render(ctx)
);
